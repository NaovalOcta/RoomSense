...
<div x-data="{
    ...
    notifications: {!! json_encode(auth()->user()->notifications()->take(5)->get()->map(fn($notif) => [
        'id' => $notif->id,
        ...
        'created_at_wib' => $notif->created_at->timezone('Asia/Jakarta')->diffForHumans()
    ])) !!},
    ...
    <div x-show="open" 
         ...
         class="absolute right-0 mt-2 w-80 md:w-96 rounded-2xl border border-slate-800 bg-slate-900/95 backdrop-blur-md shadow-2xl z-50 overflow-hidden" 
         role="menu" 
         style="display: none;">
        <div class="px-4 py-3.5 border-b border-slate-800 flex justify-between items-center">
            <h3 class="text-sm font-semibold text-white">Notifications</h3>
            <span x-show="count > 0" x-text="`${count} unread`" class="text-xs font-medium text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded-full"></span>
        </div>
            ...
            <template x-for="notif in notifications" :key="notif.id">
                <div @click="markAsRead(notif.id, notif.action_url)" 
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-slate-400 mt-0.5 leading-relaxed" x-text="notif.message"></p>
        ...
        <div class="p-2 border-t border-slate-800 bg-slate-950/40 flex items-center justify-between gap-2">
            <button x-show="count > 0" 
                    @click="markAllAsRead()" 
                    class="text-xs text-slate-400 hover:text-white px-3 py-1.5 rounded-xl hover:bg-slate-800 transition-colors">
                Mark all as read
            </button>
            <a href="{{ route('notifications.index') }}" 
               class="text-xs text-blue-400 hover:text-blue-300 font-semibold px-3 py-1.5 rounded-xl hover:bg-blue-500/10 transition-colors ml-auto">
                View all
            </a>
        </div>
    </div>
